...
import 'package:sensebox_bike/sensors/dci_sensor.dart';
...
void main() {
  ...
  group('getUiPriorityByUuid', () {
    ...
    test('returns correct priority for distance right sensor UUID', () {
      expect(getUiPriorityByUuid(DistanceRightSensor.sensorCharacteristicUuid),
          20);
    });
    test('returns correct priority for DCI sensor UUID', () {
      expect(getUiPriorityByUuid(DCISensor.sensorCharacteristicUuid), 20);
    });
    ...
    test('returns correct priority for surface anomaly sensor UUID', () {
      expect(getUiPriorityByUuid(SurfaceAnomalySensor.sensorCharacteristicUuid),
          60);
    });
  ...
  group('getUniqueSortedSensorEntries', () {
    ...
    test(
        'sorts surface classification sensors correctly when mixed with other sensors',
        () {
      final sensorData = [
        SensorData()
          ..title = 'surface_classification'
  ...
  group('getTranslatedTitleFromSensorKey', () {
    testWidgets('returns correct translation for distance sensor',
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: Builder(
            builder: (context) {
              final result =
                  getTranslatedTitleFromSensorKey('distance', null, context);
    ...
    testWidgets('returns correct translation for distance_right sensor',
        (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          localizationsDelegates: [
          ...
          home: Builder(
            builder: (context) {
              final result = getTranslatedTitleFromSensorKey(
                  'distance_right', null, context);
