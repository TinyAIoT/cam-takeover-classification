...
SurfacePostProcessor *m_surface_postprocessor = nullptr;
...
bool initialize_surface_model() {
    ...
    const int check = 5;
    m_surface_postprocessor = new SurfacePostProcessor(surface_model, check, std::numeric_limits<float>::lowest(), true);
    if (!m_surface_postprocessor) {
        ESP_LOGE("SURFACE", "Failed to create postprocessor");
        delete m_surface_preprocessor;
        delete surface_model;
        m_surface_preprocessor = nullptr;
        surface_model = nullptr;
        return false;
    }
...
bool process_surface_image(const dl::image::img_t* input_img) {
    ...
    // Add safety checks
    if (!input_img || !input_img->data) {
        ESP_LOGE("SURFACE", "Invalid input image!");
        return false;
    }
    
    if (!surface_model || !m_surface_preprocessor || !m_surface_postprocessor) {
        ESP_LOGE("SURFACE", "Model components not initialized!");
        return false;
    }
